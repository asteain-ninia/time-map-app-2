...
*   **問題**: アンドゥ/リドゥ操作時に、履歴から復元したデータが元のドメインオブジェクト（クラスインスタンス）ではなくプレーンなJavaScriptオブジェクトになってしまい、メソッド呼び出し時に `TypeError` が発生したり、期待通りに動作しなかったりした。また、履歴に保存するデータの形式（インスタンス vs プレーンオブジェクト）の選択が不適切だった。
*   **原因**:
    *   アンドゥ履歴に `JSON.parse(JSON.stringify(feature))` などでプレーンオブジェクトを保存し、Undo時にそれをそのまま `world.features` に戻していた。
    *   または、履歴にドメインインスタンスを直接保存した場合、Undo時にそのインスタンスを再利用したが、そのインスタンスの状態が他の操作によって意図せず変更されている可能性があった（イミュータビリティが破壊されている場合）。
    *   ネストされた値オブジェクト（`Property` 内の `TimePoint` など）のデシリアライズ処理が漏れていた。
*   **解決策**:
    1.  **履歴の保存形式**:
        *   **方法A（推奨）**: 履歴には、復元に必要な**プレーンオブジェクト**を保存する（`JSON.parse(JSON.stringify(...))` や、必要な情報のみ抽出したオブジェクト）。Undo/Redo時に、これらのプレーンオブジェクトから**新しいドメインインスタンスを再生成**する。メモリ使用量が抑えられ、状態の意図しない共有を防げるが、再生成ロジックが必要。
        *   **方法B**: 履歴に**ドメインインスタンスのディープコピー**を保存する。Undo/Redo時にコピーしたインスタンスを直接使用する。実装は簡単だが、メモリ使用量が増加し、コピー漏れのリスクがある。
        *   (今回は方法Bに近いがインスタンスを直接保存する形で実装し、Undo時に再利用している。今後のリファクタリングで方法Aへの移行を検討。)
    2.  **ドメインオブジェクトの再生成**: 履歴から復元する際、必ず各ドメインクラス (`Point`, `Line`, `Polygon`, `Property`, `TimePoint` など) のコンストラクタを呼び出して新しいインスタンスを生成する。
    3.  **デシリアライズヘルパー**: プレーンオブジェクトからドメインオブジェクト（特に値オブジェクト）を安全に再生成するためのヘルパーメソッド (`_deserializeTimePoint`, `_deserializeProperty` など）を実装し、利用する。
*   **教訓**:
    *   アンドゥ/リドゥ機能を実装する際は、履歴に保存するデータの形式（インスタンス vs プレーンオブジェクト）を慎重に選択し、その影響（メモリ、複雑さ、状態共有リスク）を理解する。
    *   履歴から状態を復元する際は、必ずドメインオブジェクトの**正しい型のインスタンス**を生成する。プレーンオブジェクトを直接代入しない。
    *   ネストされたオブジェクト（特に値オブジェクト）も再帰的に正しくデシリアライズ（インスタンス化）する処理を忘れない。専用のヘルパー関数やシリアライザを活用する。
