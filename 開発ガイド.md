...
*   **原因**: 要素を選択・操作するロジックが、アプリケーションの現在のコンテキスト（表示中の時間、表示レイヤー、フィルター状態など）を考慮せず、データソース全体（例: `world.vertices` 全て）を対象としている。表示状態を管理するViewModel (`MapViewModel._features` 等）と、操作対象を決定するロジック（`MapView._findClosestVertex`, `MapViewModel.selectVertex` 等）が連携していない。
*   **教訓**:
